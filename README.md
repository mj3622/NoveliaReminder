...
- `retry_max_retries`: 单次请求失败后的重试次数，默认 6，服务器/网络不稳定时可调高
- `retry_backoff_sec`: 重试间隔基数（秒），实际间隔为 1×、2×、3×…，默认 5
- `delay_between_requests`: 拉取收藏列表与文库详情时，每次请求之间的间隔（秒），默认 1.5，用于减轻被对端关连接的概率
- `notify_on_final_failure`: 是否在重试后仍失败时发送告警通知（登录失败、列表拉取异常中断、部分文库详情获取失败等），默认 true
...
3. **SSLZeroReturnError / TLS connection has been closed (EOF)**

   在服务器或网络不稳定时，对端可能中途关闭 TLS 连接，导致单次请求失败。本程序已内置**自动重试**（默认最多 3 次、退避等待），多数情况下可自动恢复。若仍频繁出现，可尝试：在 cron 中配置代理、适当拉长任务间隔，或检查服务器与目标站点的网络质量。
